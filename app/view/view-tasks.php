...
<div class="create-helpdesk-container">
    <div class="form-header">
        ...
        <p>Visualize os detalhes do chamado</p>
    </div>

    <form action="/create-tasks" method="post" class="helpdesk-form">
        <div class="form-group">
            <label for="title">Título do Chamado *</label>
            <input type="text" id="title" name="title" value="<?= $data['title'] ?>" required maxlength="100" disabled>

        </div>

        <div class="form-group">
            <label for="description">Descrição Detalhada *</label>
            <textarea type="text" id="description" name="description" required maxlength="1000" disabled><?= $data['description'] ?></textarea>
            <small class="form-hint">Máximo 1000 caracteres</small>
        </div>

        <div class="form-group">
            <label for="status">Status</label>
            <select name="status" id="status" value="<?= $data['status'] ?>" disabled>
